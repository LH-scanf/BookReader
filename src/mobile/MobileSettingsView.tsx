...
function MobileUpdatePage() { const [message, setMessage] = useState(""); const [busy, setBusy] = useState(false); const [checked, setChecked] = useState(false); const [update, setUpdate] = useState<(() => Promise<void>) | null>(null); useEffect(() => { let unsubscribe: (() => void) | undefined; void import("../pwa-update").then((pwa) => { const refresh = () => { const next = pwa.getAvailablePwaUpdate(); // A function passed directly to React state is treated as an updater. Wrap it so the PWA action itself is retained.
      setUpdate(() => next); if (next) setMessage("已发现新版本，尚未应用。点击“更新”后会重新加载应用。"); }; refresh(); unsubscribe = pwa.subscribePwaUpdate(refresh); }); return () => unsubscribe?.(); }, []); const check = async () => { setBusy(true); setMessage(""); try { const { requestPwaUpdateCheck } = await import("../pwa-update"); await requestPwaUpdateCheck(); setChecked(true); } catch (reason) { setMessage(`检查更新失败：${reason instanceof Error ? reason.message : String(reason)}`); } finally { setBusy(false); } }; const apply = async () => { if (!update) return; setBusy(true); setMessage(""); try { await update(); } catch (reason) { setMessage(`更新失败：${reason instanceof Error ? reason.message : String(reason)}。当前仍在使用已安装版本。`); setBusy(false); } }; return <section className="mobile-settings-detail"><p>{update ? "已发现并下载新版本；当前尚未更新。点击“更新”后会重新加载应用，本机书库与待同步数据会保留。" : checked ? "已检查：当前未发现新版本。" : "当前正在使用已安装版本。点击“检查更新”可确认是否有新版本。"}</p><button className="mobile-settings-primary" disabled={busy} onClick={() => void (update ? apply() : check())}>{busy ? "正在处理…" : update ? "更新" : "检查更新"}</button>{message && <p className="mobile-settings-message" role="status">{message}</p>}</section>; }
