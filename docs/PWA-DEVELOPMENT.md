...
| 增量同步 | 部分完成 | 全量枚举目录，eTag 控制变更内容下载；尚无 Graph delta 游标 |
| 测试与构建 | 本地检查通过 | 21 项 Web 测试、6 项 Rust 测试；Web 和桌面前端构建通过 |
...
5. 接入 Graph delta，完善冲突处置界面及实际两端并发测试。永久删除、手机编辑笔记、多账号迁移仍不在首版范围。
