...
| 登录 / Graph / 同步 | 已实现，真实账号联调进行中 | 撤权后 AppFolder-only 访问成功，真实建库与首本导入同步成功；下载往返与双端验收待完成 |
...
| 真机与真实云端 | 云端部分通过，真机待验收 | 本地浏览器已完成真实账号建库和导入同步；iPhone、双端进度与并发仍待验收 |
...
1. 微软 SPA 注册及 localhost 回调已完成；仍需确定稳定 HTTPS 测试域名并登记对应回调。操作步骤见 `PWA-DEPLOYMENT.md`。
2. 继续真实 Graph 验收：重新下载、桌面/手机双向进度、笔记读取、软删除/恢复、失败重试及限流。登录回跳、应用目录访问、建库和首本导入同步已通过。
